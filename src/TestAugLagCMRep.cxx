...
#include "vtkUnstructuredGrid.h" //AMP
#include "vtkThreshold.h" //AMP
#include "vtkDataSetSurfaceFilter.h" //AMP
...
public:
  ...
  MeshFunctionVolumeIntegral(CMRep *model,
    ...
    *((TriangleMesh *)(&B)) = model->bnd_tri;
    ...
    w->SetInputData(test);
    char buffer[64];
    printf("Saving the submesh with %zu\n label", label);
    sprintf(buffer,"subflatLabel%zu.vtk", label);
...
public:
  ...
  DiceOverlapComputation(
    ...
    double vol_image, TFunction &in_func, size_t label)
    : integrator(model, n_wedges, sub_level, flat_subdivision_mode, in_func, label)
    {
    this->vol_image = vol_image;
    }

  diceCalc Compute(const Matrix &qb, const Matrix &qm) //previous return type: double
    {
    integrator.Compute(qb, qm, v, fv);
    diceCalc diceVar;

    //printf("v, fv and dice %f %f %f\n", v, fv, 2.0 * fv / (v + vol_image));
    diceVar.numerator = fv;
    diceVar.denominator = v + vol_image;
    return diceVar;
    ...
    void ComputeAdjoint(
    const Matrix &qb, const Matrix &qm,
    double d_obj__d_num, double d_obj__d_den,
    Matrix &d_obj__d_qb, Matrix &d_obj__d_qm)
    {
    //double d_obj__d_fv = d_obj__d_dice * 2.0 / (v + vol_image);
    //double d_obj__d_v = - d_obj__d_dice * 2.0 * fv / ((v + vol_image) * (v + vol_image));
    integrator.ComputeAdjoint(qb, qm, d_obj__d_den, d_obj__d_num, d_obj__d_qb, d_obj__d_qm);
    }

    double getVolumeImage()
    {
      return vol_image;
    }
...
template <class TFunction> //size_t N ,int N
class GeneralizedDiceOverlapComputation
{
public:
  typedef vnl_matrix<double> Matrix;
  typedef vnl_vector<double> Vector;

  GeneralizedDiceOverlapComputation(CMRep *model, unsigned int n_wedges, unsigned int sub_level, bool flat_subdivision_mode,
    DiceOverlapComputation<TFunction>* dicer[], size_t label, int numLabels)//, TFunction &in_func_temp) //TFunction &in_func[] TFunction* (*in_func)[N]
  //: integrator(model, n_wedges, sub_level, flat_subdivision_mode, in_func_temp, label) //should pass one binary image to compute fv,v
    {
    this->numLabels = numLabels;
    this->in_func_class = dicer;
    //this->sumNumerator = sumNumerator;
    //this->sumDenominator = sumDenominator;

    //printf("Constructor of GeneralizedDiceOverlapComputation \n");
    }

    double Compute(const Matrix &qb, const Matrix &qm)
      {
        double diceGen = 0.0;
        double weightVolume = 0.0;
        double dice_per_label = 0.0;
        ...
        for(int l =1; l<= this->numLabels; l++)
        {
          weightVolume = 1.0 / (in_func_class[l]->getVolumeImage() * in_func_class[l]->getVolumeImage());

          numo = in_func_class[l]->Compute(qb, qm).numerator;
          deno = in_func_class[l]->Compute(qb, qm).denominator;

          sumNumerator += weightVolume * numo;
          sumDenominator += weightVolume * deno;

          //printf("sumNumerator %f sumDenominator %f\n", sumNumerator, sumDenominator);

          dice_per_label = 2.0 * (numo / deno);
      ...
      void ComputeAdjoint(const Matrix &qb, const Matrix &qm,
          double d_obj__d_gendice,
          Matrix &d_obj__d_qb, Matrix &d_obj__d_qm)
      {
      d_obj__d_qb.fill(0.0); d_obj__d_qm.fill(0.0);
      Matrix d_obj__d_qb_temp = d_obj__d_qb, d_obj__d_qm_temp = d_obj__d_qm;

      for(int l =1; l<= this->numLabels; l++)
      {
        //printf("sumNumerator %f sumDenominator %f\n", sumNumerator, sumDenominator);

        double weight_label = 1.0 / (in_func_class[l]->getVolumeImage() * in_func_class[l]->getVolumeImage());

        double d_obj_d_ni = 2.0 * (weight_label * d_obj__d_gendice) / sumDenominator ;
        double d_obj_d_di = -2.0 * (sumNumerator * d_obj__d_gendice * weight_label) / (sumDenominator * sumDenominator) ;

        //d_obj_d_ni = d_obj_d_ni * weight_label;
        //d_obj_d_di = d_obj_d_di * weight_label;

        in_func_class[l]->ComputeAdjoint(
          qb, qm, d_obj_d_ni, d_obj_d_di, d_obj__d_qb_temp, d_obj__d_qm_temp);

        d_obj__d_qb += d_obj__d_qb_temp;
        d_obj__d_qm += d_obj__d_qm_temp;
      }
      }
  ...
  private:
    ...
    double v, fv, vol_image, numLabels;
    DiceOverlapComputation<TFunction>** in_func_class;
...
public:
  ...
  ImageDiceFunction(ImageType *im, double sigma, int num_label)
    {
      // ImageDiceFunction(const char *fname, double sigma, int num_label)
    typedef FastLinearInterpolator<ImageType, double, 3> InterpolatorType;

    m_InputImage = im;

    // Binarize the input image
    printf("Binarizing the input image for label %d\n", num_label);

    int LowerThreshold = num_label;
    int UpperThreshold = num_label;
    int OutsideValue = 0;
    int InsideValue = 1;
    ...
    char buffer[64];
    sprintf(buffer,"targetImageLabel%d.nii.gz",num_label);
    ...
    fltSmooth->SetInput(filter->GetOutput());
    ...
    printf(">>>>>>>>Volume of Target Image %f \n", m_Volume);
...
/*
    int nt = model->bnd_vtk->GetNumberOfCells();
    printf("%d %d\n", nt, model->nv);
    vtkDataArray *b_label = model->bnd_vtk->GetCellData()->GetArray("Label");
    Vector Label(nt);

    for(int i = 0; i < nt; i++)
      {
        int label = b_label ? b_label->GetTuple1(i) : NOID;
        Label[i] = label;
      }

    vmbb.AddArray(Label, "Label");
*/
...
int main(int argc, char *argv[])
{
  ...
  int nt = m_template.bnd_vtk->GetNumberOfCells();
  vtkDataArray *b_label = m_template.bnd_vtk->GetCellData()->GetArray("Label");
  int max_label = 0;

//TODO: set of labels
  for(int i = 0; i < nt; i++)
    {
      auto label = b_label ? b_label->GetTuple1(i) : -1; //NOID
      if(label == -1)
      {
        break;
      }
  ...
  printf("Number of labels in the vtk mesh %d\n", max_label);
  int numLabels;
  ...
  reader->SetFileName(fn_target_image.c_str());
  reader->Update();

  // create an object for each binary input image

  //ImageDiceFunction** binaryImages = new ImageDiceFunction*[numLabels];
  //ImageDiceFunction *binaryImages = new ImageDiceFunction[numLabels];

  ImageDiceFunction* binaryImages[numLabels];

  for(int l=1; l<=numLabels; l++)
  {
    if (numLabels == 1)
    {
      binaryImages[l] = new ImageDiceFunction(reader->GetOutput(), param.image_sigma, 1);
    }
    else
    {
      printf("Creating an object for label %d\n", l);
      binaryImages[l] = new ImageDiceFunction(reader->GetOutput(), param.image_sigma, l);
      printf("%s\n",  typeid(binaryImages[l]).name());
    }
  }
  ...
  // Some Tests (assuming there is a valid image)
  if(param.check_deriv)
    {
    ...
    for(int l=1; l<=numLabels; l++)
      {
        printf("Testing Dice derivatives for label %d\n", l);
        TestDice(&m_template, *binaryImages[l], binaryImages[l]->GetVolume(), 1e-4);
      }
    }

  // Initialize DiceOverlap computer object for each label

  typedef DiceOverlapComputation<ImageDiceFunction> DiceOverlapType;
  // DiceOverlapType** dicerCompute = new DiceOverlapType*[numLabels];
  DiceOverlapType* dicerCompute[numLabels];

  for(int l=1; l<=numLabels; l++)
  {
    if (numLabels == 1)
    {
      dicerCompute[l] = new DiceOverlapType(&m_template, 5, param.loop_subdivision_level,
                                  !param.limit_surface_constraints, binaryImages[l]->GetVolume(), *binaryImages[l], NOID);
    }
    else
    {
      printf("Creating an DiceOverlap object for label %d \n", l);
      dicerCompute[l] = new DiceOverlapType(&m_template, 5, param.loop_subdivision_level,
                                  !param.limit_surface_constraints, binaryImages[l]->GetVolume(), *binaryImages[l], l);
    }
  }

  printf(">>>>>call generalizedDice HERE<<<<<\n");
  typedef GeneralizedDiceOverlapComputation<ImageDiceFunction> gdice; // ,2>
  gdice *genDice = NULL;
  genDice = new gdice(&m_template, 5, param.loop_subdivision_level, !param.limit_surface_constraints, dicerCompute, 1, 2); //, *binaryImages[1]);

  //genDice->Compute();

  //typedef GeneralizedDiceOverlapComputation<ImageDiceFunction> sample;
  //sample *cider = NULL;
  //cider =  new sample(binaryImages, numLabels);

  //*dicerCompute[1].Compute();

  // Check Parameters and TODO: pass the generalized DiceOverlap
  if(param.use_slack)
    {
    // Define the traits for the AL objective
    typedef PointBasedMediallyConstrainedFittingTraits<gdice> TraitsType;
    do_optimization<TraitsType>(param, &m_template, &m_target, genDice); //dicer
    }
  else
    {
    // Define the traits for the AL objective
    printf(">>>>>>>PointBasedMediallyConstrainedWithoutSlackFittingTraits<<<<<<\n");
    typedef PointBasedMediallyConstrainedWithoutSlackFittingTraits<gdice> TraitsType;
    do_optimization<TraitsType>(param, &m_template, &m_target, genDice); //dicer //dicerCompute[1]
    }

    // Delete all the image pointer
    for(int l=1; l<=numLabels; l++)
    {
      if(binaryImages[l])
        delete binaryImages[l];
    }


  //ImageDiceFunction *idf = NULL;
  //if(fn_target_image.size())
  //  idf = new ImageDiceFunction(fn_target_image.c_str(), param.image_sigma, num_label);
  ...
  if(idf)
    dicer = new DiceOverlapType(&m_template, 5, param.loop_subdivision_level,
                                !param.limit_surface_constraints, idf->GetVolume(), *idf, label=label);
